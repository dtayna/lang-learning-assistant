...
export class ProgressManagerComponent implements OnInit {
  ...
  onSubmit() {
    ...
    const hourPayload: InsertTimeProgress = {
      exposureHours: this.timeService.timeStringToFloat(
        formValue.exposureHours.toString(),
      ),
  ...
  onCheckboxChange(event: any) {
    ...
    if (event.target.checked) {
      ...
      const index = this.exposureTypeArray.controls.findIndex(
        (x) => x.value === value,
      )
  ...
  getLevelByHours(): CEFRLevel {
    return this.totalExposureHours > 0
      ? this.progressConfigService.getLevelFromExposureHours(
          this.totalExposureHours,
        )
      : 'A1'
  }
